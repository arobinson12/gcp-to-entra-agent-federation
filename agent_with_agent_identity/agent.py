"""
Agent 1: Native GCP Agent Identity with Service Account Impersonation & Entra ID Federation.

Architecture:
1. Deployed to Vertex AI Agent Engine with native Agent Identity (`spec.identity_type = AGENT_IDENTITY`).
2. The runtime's ambient GCP Agent Identity calls the GCP IAM Credentials API to mint a signed OIDC ID token
   for a target Service Account (`GCP_FEDERATION_SERVICE_ACCOUNT`).
...
# ==============================================================================
TARGET_FEDERATION_SA = os.environ.get("GCP_FEDERATION_SERVICE_ACCOUNT", "entra-federation-sa@your-gcp-project-id.iam.gserviceaccount.com")
...
def _get_entra_token(scope: str) -> str:
    """Acquires a GCP OIDC ID Token via target SA impersonation and exchanges it for an Entra Agent ID Access Token."""
    # 1. Obtain ambient credentials of the native GCP Agent Identity in Agent Engine
    credentials, _ = google.auth.default(scopes=["https://www.googleapis.com/auth/cloud-platform"])
    auth_req = google.auth.transport.requests.Request()
    credentials.refresh(auth_req)

    # 2. Impersonate the target Federation SA to generate a signed GCP OIDC ID Token
    oidc_url = f"https://iamcredentials.googleapis.com/v1/projects/-/serviceAccounts/{TARGET_FEDERATION_SA}:generateIdToken"
    headers = {
...
# ==============================================================================
root_agent = Agent(
    name="EntraAgentIdentityClient",
    model="gemini-3.7-flash",
    description="Multi-cloud AI agent utilizing GCP Native Agent Identity and Microsoft Entra Agent ID Blueprint Federation.",
    instruction=(
        "You are an enterprise AI assistant running on Google Cloud Vertex AI Agent Runtime. "
        "You authenticate to Microsoft Entra ID using native GCP Agent Identity federated through an Agent Blueprint and Entra Agent ID. "
